...
`tools/list` at `https://gateway.pipeworx.io/iso-codes/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/iso_codes_country \
  -H 'Content-Type: application/json' \
  -d '{"query":"United States"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/iso_codes_country`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
